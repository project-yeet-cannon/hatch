...
function Get-ProbeSection {
    ...
    .DESCRIPTION
        One SSH round trip collects everything stage 2 reasons about, rather
        than eight. That matters less for speed than for consistency: eight
        round trips are eight moments at which the node could change under
        the decision being made about it, and "the disk was empty when I
        looked and had a filesystem when I wrote to it" is not a race worth
        leaving open in a script whose next action is mkfs.
    #>
    param(
