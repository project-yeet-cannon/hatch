...
- **`MainActivity`** ([MainActivity.kt](../apps/kiosk/app/src/main/java/family/landis/aeriekiosk/MainActivity.kt)) creates a `GeckoView`/`GeckoSession` pointed at `DASHBOARD_URL` (`https://kiosk.landis.family/`) and:
  ...
  - `KioskProgressDelegate` + `KioskContentDelegate` + `scheduleRetry()` implement reconnect: on load failure, on a content-process crash, or on a page that "loaded" and turned out to be an error page, put [`KioskErrorView`](../apps/kiosk/app/src/main/java/family/landis/aeriekiosk/KioskErrorView.kt) up and retry with exponential backoff (2s → 30s cap), reset to the initial delay once a health probe confirms the origin. The last two of those three were silent holes until recently — see [Degrading gracefully](#degrading-gracefully).
...
A wall tablet has no keyboard, no console, and nobody standing at it. The dashboard has always been careful about *data* it doesn't have — a zone with no reading renders `—`, an empty agenda renders nothing, a house with no Immich has no hole where the photos would be. What it was not careful about was *itself*: every layer between the power button and a rendered snapshot failed into a blank screen, and none of them said so. Someone found out hours later by walking past a white rectangle.

The rule that shapes all of it: **every layer must be able to draw a clock.** The time of day is the one thing the wall can always be right about — it needs no network, no API and no bundle — so it is the floor every error state stands on, and a tablet showing the time with a sentence under it is a tablet still doing part of its job.

Four layers, and they cover disjoint failures rather than overlapping ones. Each is the *only* thing that can see its own case:

| Layer | Sees | Cannot see |
|---|---|---|
| **Native error screen** ([`KioskErrorView`](../apps/kiosk/app/src/main/java/family/landis/aeriekiosk/KioskErrorView.kt)) | The document never arriving — DNS, TLS, a 502, a dead content process | A bundle that broke after a document loaded fine |
| **Boot fallback** (markup inside `#root` in [`index.html`](../src/Aerie.Web/apps/dashboard/index.html)) | A bundle that 404s, fails its MIME check, or throws before React mounts | Anything, if the document itself never arrived |
| **`ErrorBoundary`** | A render-time throw from a tree that mounted | A bundle that never ran |
| **Health dot** ([`HealthDot`](../src/Aerie.Web/apps/dashboard/src/components/HealthDot.tsx)) | Everything after mount that fails without blanking the screen | Anything before mount |

The boot fallback is inline markup and classic JS in a file no build step touches, so it renders with zero stylesheets and zero modules — which is the entire point, since the case it exists for is "none of that arrived". It fades in at 600ms so a healthy mount never shows it and the wall gains no dark flash on the happy path; `main.tsx` dismisses it explicitly rather than letting `createRoot()` clear the container, because it owns a clock interval and a 10s mount watchdog that have to be torn down. That watchdog logs `kiosk mount watchdog fired`, which is the durable version of "the bundle never ran". [`bootFallback.test.ts`](../src/Aerie.Web/apps/dashboard/src/bootFallback.test.ts) guards its edges — inline JS nothing imports and nothing type-checks has no other test surface, and every line it asserts is one someone could plausibly delete while tidying.

### The two things GeckoView will not tell you

Both were open holes, both are now closed, and **both are the kind of thing a later reader removes as redundant**:

- **A content-process crash reaches no delegate you have registered by default.** `MainActivity` registers a `ContentDelegate` only to work around [bug 1758212](https://bugzilla.mozilla.org/show_bug.cgi?id=1758212), and while it was an empty object `onCrash()`/`onKill()` were the default no-ops — so a dead content process left the view blank *forever*. `onLoadError` does not fire, `onPageStop` does not fire, `scheduleRetry()` is never entered. It was the only failure here with no recovery path short of a power cycle. `KioskContentDelegate` now closes, reopens and re-attaches the session, and retries through the ladder rather than immediately, because a crash loop has to back off like anything else or it becomes a reboot loop with a wall attached.
- **An HTTP error is a *successful* load.** `onLoadError` fires for DNS, TLS and connection failures. It does not fire for a 502 from the ingress or a 500 from the API — those are ordinary responses with an error body, so `onPageStop(success = true)` would run, the backoff would reset, and the error screen would be *hidden* over whatever the proxy served. The shell believed it was connected. So a successful page stop now costs one `HttpURLConnection` GET against `/api/app-version/dashboard`, off the main thread, and **that** is what clears the screen. A WebExtension port would answer the same one bit of information with a far larger mechanism and with coupling to the bundle — the half most likely to be what is broken.

  **That probe holds no credential, and the wall has to know it.** The `HttpURLConnection` shares no cookie jar with the GeckoView, exactly like the sun-events call the backlight runs on, so `/api/app-version` has to sit on [`AuthGate`](../src/Aerie.Api/Services/Auth/AuthGate.cs)'s allow-list. It did not between 2026-08-28 and 08-31, and the failure is worth stating because it inverts the feature: the probe read *its own* 401 as "the origin is unwell", raised the error screen over a dashboard that was rendering perfectly, and retried that every 30s for three days. Tapping **Reload** worked every time and changed nothing — the page reloaded fine, the probe was refused again, and the screen came back within the second. So the shell now treats a 401/403 as the one status that says nothing about the page and falls back to Gecko's own verdict: a refusal proves the origin was healthy enough to run the gate, and the proxy error page this probe exists to catch would have answered 502 here too. The allow-list entry is the fix; that branch is what keeps the next auth change from taking every wall in the house down silently.

Failure classification and the retry ladder live in [`LoadFailures.kt`](../apps/kiosk/app/src/main/java/family/landis/aeriekiosk/LoadFailures.kt) as pure functions, apart from the activity, so both can be asserted rather than eyeballed: a tablet showing the wrong sentence still looks like it is working, and a backoff that does not back off only reveals itself as a warm tablet and a busy log. The error screen names a *category* — the network, a certificate, unreadable content, Aerie not answering, a crash — because those are five different things to do next, and carries the version and device id in a footer so a photograph of the screen is a diagnosis. Its **Reload** button is the only reload in the system that can truly bypass the cache (`LOAD_FLAGS_BYPASS_CACHE`), which is why the button belongs in the shell and not on a page whose bundle may be the problem.

### Why the fallback screen is not paranoia

Over nine days of `aerie-logs` (queried 2026-08-28), **eight dashboard sessions produced exactly one log line, ever**: `kiosk index.html parse started` fired from the inline beacon, and nothing was ever heard from that page again — no module evaluation, no mount, no poll, no error. Against 87 document parses, 85 reached `main.tsx`.

They arrive in bursts — four within 83 seconds on 08-21, four within six minutes on 08-25. One tablet failing once does not make four sessions; a tablet *retrying* does, each retry dying at the same point, which is why the wall stays white rather than recovering. And the 08-25 burst sits in the same hour as three of the fourteen genuine `Dashboard data load failed` lines (500s, 502s and network errors; the 678-line total is overwhelmingly the 08-21 401 storm that [`signIn.ts`](../src/Aerie.Web/apps/dashboard/src/lib/signIn.ts) fixed). So the sequence is: **the API goes unhealthy → the page reloads → the reload lands on a document/bundle mismatch → white → retry → white.** The blank screen and the silent API failure were never two problems.

That query is also why the health dot exists in the form it does. Not "the API might fail" but "the API failed fourteen times that week and nobody watching the wall could have known."

### The health dot

Every failure path in the dashboard already calls `clientLogger.error` — seventeen call sites across the hooks, the components and the `ErrorBoundary`, plus the window `error` (capture-phase, so resource load failures are included) and `unhandledrejection` listeners. [`clientLogger`](../src/Aerie.Web/apps/dashboard/src/lib/clientLogger.ts) therefore keeps a bounded ring of those and publishes it, and the dot is a subscriber. **That is the whole design**: a complete error feed for the price of one module change, with no new instrumentation to keep in step as components come and go. Console errors and network failures join it the same way, through wrappers installed at module init — the fetch wrapper is what lets the modal say "502 from `/api/dashboard`" rather than only "something failed".

Two details in that wrapper are load-bearing. It excludes `/api/ui-logs`, or a logging outage would log itself forever. And it collapses repeats through [`logCapture.ts`](../src/Aerie.Web/apps/dashboard/src/lib/logCapture.ts)'s dedupe, whose **window must stay comfortably longer than `App`'s poll interval** — the two are only correct relative to each other, the same way [`kioskIdleTimings.ts`](../src/Aerie.Web/apps/dashboard/src/lib/kioskIdleTimings.ts)'s two rungs are. At 60s each they were exactly equal, a failing poll landed on the boundary every time, and the dedupe collapsed nothing at all in the one case it was written for.

The dot **decays rather than blinking** ([`healthSignal.ts`](../src/Aerie.Web/apps/dashboard/src/lib/healthSignal.ts)): full strength on a fault, stepping down at three, six and nine consecutive clean polls, then gone. A binary dot flaps with the API and forgets a problem that was real five minutes ago; nine minutes at the 60s poll is long enough to still be there when someone comes back to look. Errors raise it and **warnings deliberately do not** — `Motion event stream dropped; EventSource will retry` fired 140 times in that same window and recovers on its own every time, and a dot that is up for those is a dot nobody reads. Warnings still appear in the modal, which is where a drifting device clock surfaces.

It sits top-left because the right half of the header is a 108px clock, and a warning indicator should not compete with the thing the wall is most read for. Its red is stated absolutely and mixed into `--card`, the [`AlertBanner`](../src/Aerie.Web/apps/dashboard/src/components/AlertBanner.tsx) trick for the `AlertBanner` reason: the hue must mean the same thing at every hour, and a fixed red dot would be the brightest thing in the house at 3am.

### How old is that number

`ZoneClimate.CurrentTempF` is the newest measurement anywhere in the nine-hour history window, so a sensor that stopped reporting at noon still produces a confident number at 5pm. `CurrentAsOf` is what lets a client tell a live house from a dead one, and it is **the timestamp of the row the value came from, and only of a row** — null when the value fell back to a history bucket, because a bucket boundary is not a moment anything was measured and returning it would make an interpolated value look fresher than the data under it. That null is the distinction a later reader is most likely to "fix"; it is pinned by [`ZoneServiceCurrentAsOfTests`](../src/Aerie.Api.Tests/Dashboard/ZoneServiceCurrentAsOfTests.cs).

[`staleness.ts`](../src/Aerie.Web/apps/dashboard/src/lib/staleness.ts) turns that into three states — fresh under ten minutes, stale from ten to twenty, no data past twenty. Ten rather than five because `SampleChannels` polls every minute but reads Home Assistant's *recorder*, so the cadence that matters is the device's, and a five-minute floor would call a thermostat reporting every few minutes broken most of the day. Stale dims the number and the line and dates the reading in the expanded body; the status and labels are untouched, because a fifteen-minute-old reading is still the best answer anyone has. One guard covers both ways of having nothing to say — never reported, and stopped reporting — so the two cannot drift into different-looking states that mean the same thing; they differ only in the sentence.

**The comparison is against `nowOnServerClock`, not the snapshot's `generatedAt`, and that is not a refinement.** Measured against `generatedAt` verbatim, every reading's age freezes the moment the API dies: `generatedAt - currentAsOf` is a constant once both stop moving, so a wall whose API failed an hour ago would report every bar as fresh — the exact confident-but-wrong screen this work exists to prevent, inside the code meant to prevent it. `App` advances the snapshot's clock by how long the page has held it, re-derived on the 15s tick, so each bar walks fresh → stale → no data on its own while the poll keeps failing. Both terms of the elapsed subtraction come from the device clock and both timestamps in the comparison from the server's, so neither clock's drift leaks into the other. `?source=mock`'s three zones report at 1, 14 and 45 minutes so all three states are visible without unplugging a sensor.

The device clock gets its own check ([`clockSkew.ts`](../src/Aerie.Web/apps/dashboard/src/lib/clockSkew.ts)) for the obvious reason: it is the floor the fallback screen, the error screen and the header all stand on, and a tablet that has been offline can drift. It is logged as a warning and **not corrected** — a wall that silently disagrees with the phone in your hand is the bug, and papering over it removes its only visible symptom.
...
Each of these is a failure the layers above claim to handle, and each is reachable without breaking anything:

| Do this | Expect |
|---|---|
| Rename a hashed asset on the server | The in-page fallback with a live clock, not white |
| Stop the API pods, leave the ingress up | The native error screen naming the status — not a proxy error page under a hidden overlay |
| Block egress at the router | First paint at the usual speed, in a fallback face |
| Disable one HA entity | That bar dims at ten minutes and reads "no data" at twenty |
| Let the API 500 once | The dot at full strength, fading over the next nine polls |

A content-process crash is the one that resists being staged: `chrome://crashcontent` is unreachable under the navigation delegate's host check, so it takes memory pressure to force, and is otherwise covered by review and by the `GeckoView content process ended` log line.
