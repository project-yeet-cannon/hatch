...
- **Typing counts as presence.** Soft keyboards do not reliably produce `keydown`, so anything with an idle timeout has to add `input` to [`ACTIVITY_EVENTS`](../src/Aerie.Web/apps/dashboard/src/lib/kioskLifecycle.ts) — a display that resets while someone is typing is the failure this whole section exists to prevent.
