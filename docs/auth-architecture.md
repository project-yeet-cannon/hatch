...
| `trading.${DOMAIN}` | Yes, at `/` | The trading silo's control plane ([`trading.md`](plans/trading.md)). It is in its own namespace, so it declares its **own** `Middleware` rather than annotating against the chart's — an annotation names a middleware as `<namespace>-<name>@kubernetescrd`, and reaching across namespaces would need Traefik's `allowCrossNamespace`. That costs nothing, because a `forwardAuth` address is a URL rather than a namespaced reference: both objects call the same `/api/auth/verify`, and there is still one `AuthGate` deciding. It is unconditional rather than gated on `auth.mode`, which it can afford to be — `EvaluateAsync` returns `Exempt` when `Auth:Enabled` is false, so at `none` it answers 204 to everything. **The consequence worth knowing before it is reported as a bug:** `Verify` rebuilds its redirect origin from `X-Forwarded-Host`, so an un-enrolled browser is bounced to `/apps/auth/` on `trading.`, which serves no sign-in shell. That service redirects the path to the real one and the cookie's domain covers both hosts, so it is an extra hop rather than a dead end. |
...
| `/api/aerie-revision` | Which commit this replica is running. The caller who most needs it is the one holding a stale build that is about to be refused, so gating it hides the answer from exactly that caller. The cluster's own state is withheld separately, by the controller. |
...
| `/api/app-version/*` | **The second one that bites, and it did.** The same shell polls this over the same cookie-less `HttpURLConnection` to decide whether the page it just loaded is the dashboard or the proxy's error page. Gating it does not degrade the probe — the shell reads the 401 as *"Aerie is reachable but isn't answering"*, covers a dashboard rendering perfectly well underneath, and retries that forever on a wall tablet nobody power-cycles. It was off this list from 2026-08-28 to 08-31 and did exactly that. The answer is a list of the bundle's content-hashed asset filenames, strictly less than the row two above already hands out. |
