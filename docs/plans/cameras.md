...
- [~] From inside the cluster, `curl http://go2rtc:1984/api/frame.jpeg?src=<entity id>` returns a JPEG. **Done off-cluster instead, and it passed**: the same image, from the same go2rtc version with the same config, fetched from the dev Mac. That settles the camera half of the split — the remaining question is not "does the camera work" but "can a pod reach it", which is the egress item above. This is the clean split point: a frame here means the camera half works and anything still broken is Aerie's; no frame means the camera half is, and none of Phase 7 or 8 is implicated.
