...
- [x] `[verify]` 3. Confirm filtering/search works (by container, level, free text) at `logs.${DOMAIN}`.
- [x] `[code]` 4. Create Caddy subdomains `logs.landis.family` for opensearch and `status.landis.family` for kuma (make a suggestion if there would be more appropriate names)
...
- 2026-07-26: Phase 3 step 4 — found already satisfied, no new code needed. Per `reverse-proxy-architecture.md`, exposing a subdomain is entirely: (a) a `caddy: <name>.${DOMAIN}` + `caddy.reverse_proxy` label pair, and (b) attaching the service to the `edge` network — no separate DNS step (pfSense's wildcard override + Route53 DNS-01 handle that automatically). Both were already done in earlier steps: `uptime-kuma` got `status.${DOMAIN}` + `edge` in Phase 1 step 1, and `opensearch-dashboards` got `logs.${DOMAIN}` + `edge` in Phase 3 step 1. `cd.yml` already has `DOMAIN` wired from `vars.DOMAIN`. The suggested names (`logs`/`status`) match what's already deployed, so no rename was needed either. Just checked the box.
