...
public sealed class HatchClientTests
{
    ...
    public async Task A_key_is_sent_as_a_bearer_and_no_runner_header_goes_with_it()
    {
        ...
        using var client = new HatchClient(Settings("aerie_ak_test"), "test:/checkout", wire);

        await client.Send(HttpMethod.Get, "/api/hatch/board", null, default);

        Assert.True(client.Keyed);
        Assert.Equal("Bearer aerie_ak_test", wire.Authorization);
    ...
    public async Task A_401_against_a_key_that_was_sent_says_the_key_was_not_accepted()
    {
        ...
        using var client = new HatchClient(Settings("aerie_ak_wrong"), "test:/checkout", wire);
    ...
    {
        using var client = new HatchClient(Settings("aerie_ak_test"), "test:/checkout", new DeadWire());
