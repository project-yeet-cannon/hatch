...
public sealed class RestartTests : IDisposable
{
    ...
    public RestartTests()
    {
        ...
        Put("src/Aerie.Hatch/GoToWork.cs", "// the loop\n");
        Put("src/Aerie.Hatch/Program.cs", "// the entry point\n");
        Put("src/Aerie.Hatch.Contracts/Dtos.cs", "// the wire\n");

        // Everything the loop is not: another project's source, and its own
        // build output.
        Put("src/Aerie.Api/Program.cs", "// somebody else's\n");
        Put("src/Aerie.Hatch/bin/Release/net10.0/hatch-runner.dll", "compiled\n");
        Put("src/Aerie.Hatch/obj/project.assets.json", "{}\n");
    }
    ...
    public void It_reads_the_loop_and_the_wire_and_the_script_and_nothing_else()
    {
        ...
        Assert.Contains("src/Aerie.Hatch/GoToWork.cs", print.Files.Keys);
        Assert.Contains("src/Aerie.Hatch.Contracts/Dtos.cs", print.Files.Keys);

        // Another project's source is not this loop, and a change to it is not
        // one this loop has to come back for.
        Assert.DoesNotContain("src/Aerie.Api/Program.cs", print.Files.Keys);
    }
    ...
    public void Build_output_is_not_source_and_does_not_enter_into_it()
    {
        ...
        Put("src/Aerie.Hatch/bin/Release/net10.0/hatch-runner.dll", "compiled again\n");
        Put("src/Aerie.Hatch/obj/Release/net10.0/GoToWork.g.cs", "// generated\n");
    ...
    {
        var before = Take();
        Put("src/Aerie.Hatch/GoToWork.cs", "// the loop, improved\n");
        var after = Take();

        Assert.NotEqual(before.Digest, after.Digest);
        Assert.Equal(["src/Aerie.Hatch/GoToWork.cs"], after.ChangedFrom(before));
    }
    ...
    {
        var before = Take();
        Put("src/Aerie.Hatch/Restart.cs", "// new\n");
        var after = Take();

        Assert.NotEqual(before.Digest, after.Digest);
        Assert.Equal(["src/Aerie.Hatch/Restart.cs"], after.ChangedFrom(before));
    }
    ...
    {
        var before = Take();
        File.Delete(Path.Combine(_root, "src", "Aerie.Hatch", "Program.cs"));
        var after = Take();

        Assert.NotEqual(before.Digest, after.Digest);
        Assert.Equal(["src/Aerie.Hatch/Program.cs"], after.ChangedFrom(before));
    }
    ...
    {
        var before = Take();
        Put("src/Aerie.Hatch/Program.cs", "// changed\n");
        Put("scripts/hatch.sh", "# changed\n");

        Assert.Equal(["scripts/hatch.sh", "src/Aerie.Hatch/Program.cs"], Take().ChangedFrom(before));
    }
