...
public static class Prompt
{
    ...
    [
        ...
        "",
        "```",
        $"./scripts/hatch.sh show {key}              the ticket and its comments",
        $"./scripts/hatch.sh start {key}             move it to in progress",
        $"./scripts/hatch.sh move {key} <column>     move it anywhere non-terminal",
        $"./scripts/hatch.sh comment {key} \"...\"     write on the ticket",
        $"./scripts/hatch.sh ask {key} \"...\"         ask for a decision, and stop",
        $"./scripts/hatch.sh api GET /api/hatch/issues?parentKey={key}",
        "```",
        "",
