...
import { IDLE_DIM_AFTER_MS, IDLE_TIMEOUT_MS, STANDBY_AFTER_MS } from './kioskIdleTimings';
...
describe('idle ladder', () => {
  it('escalates: reset, then dim, then standby', () => {
    expect(IDLE_TIMEOUT_MS).toBeLessThan(IDLE_DIM_AFTER_MS);
    expect(IDLE_DIM_AFTER_MS).toBeLessThan(STANDBY_AFTER_MS);
  });
});
