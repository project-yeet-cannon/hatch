...
// The referrer is redacted for the same reason `url` is below: arriving at
// the manual form from a failed deep link makes the previous URL - the one
// carrying the invite code - the referrer.
...
function buildEntry(level: LogLevel, message: string, extra?: Record<string, unknown>): OutgoingEntry {
  return {
    ...
    sessionId,
    deviceId,
    // Not location.href: on /apps/auth/r/<code> that string is a live
    // credential (see redactUrl).
...
// This shell runs on whatever phone or tablet is being enrolled, where there
// is no devtools window to open and the person holding it is already stuck -
// so console.warn/console.error calls (React warnings, a failed fetch's
// details) are mirrored into the shipped log stream too, while still calling
// through so devtools behave normally where they do exist.
