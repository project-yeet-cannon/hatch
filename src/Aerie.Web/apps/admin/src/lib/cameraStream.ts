 ...
 * from the modal so the parts with decisions in them can be tested without a
 * MediaSource, a WebSocket or a camera - the same split
 * MotionEventStream/MotionEventsController makes on the API side.
 *
 * A verbatim copy of apps/dashboard/src/lib/cameraStream.ts, which is where its
 * tests live - the apps have no shared package, and clientLogger/signIn/
 * deviceMetadata are already carried the same way. Both apps relay the same
 * protocol from the same endpoint, so a change to one belongs in the other.
 *
