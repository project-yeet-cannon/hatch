 ...
 * cost hls.js plus seconds of latency, on a view whose entire job is to show
 * what is happening at the door right now.
 *
 * A verbatim copy of apps/dashboard/src/hooks/useCameraStream.ts bar its import
 * paths - see lib/cameraStream.ts for why the two apps carry copies. The
 * decisions live there and are tested there; this is the imperative shell
 * around them.
 */
export function useCameraStream(deviceId: string): {
  ...
  useEffect(() => {
    ...
    const openSocket = () => {
      ...
      // Same origin as the app, so the page needs no camera address of its own
      // and the relay inherits whatever auth it already passed.
    ...
    void video.play().catch(() => {
      // Interrupted by the view closing, usually. The catch keeps it out of
    ...
    return () => {
      ...
      // camera's - rather than leaving one open per feed this tab has ever
      // shown.
