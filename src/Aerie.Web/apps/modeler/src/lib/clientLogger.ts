...
function send(entries: OutgoingEntry[], useBeacon: boolean): void {
  ...
  fetch(ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body, keepalive: true }).catch(() => {
    // Best-effort - there's nowhere left to report a logging failure to. In
    // particular this is expected to fail during `npm run dev` (no Aerie.Api
    // running), since modeler has no other dependency on the API.
  });
}
