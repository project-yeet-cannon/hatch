...
```
docker compose exec -T db pg_dump -U user --clean --if-exists aerie > hatch-backup.sql
```
...
docker compose exec -T db psql -U user -d aerie -v ON_ERROR_STOP=1 < hatch-backup.sql
...
```
Get-Content hatch-backup.sql | docker compose exec -T db psql -U user -d aerie -v ON_ERROR_STOP=1
```
