/* The bar, for a page Aerie did not build.
   ---------------------------------------------------------------------------
   Two pages in the house are not Vite/React apps and cannot render <TopBar>:

   - **Swagger UI**, which is Swashbuckle's page. Aerie owns the document only
...
declare global {
  interface Window {
    ...
    aerieTopBar?: StandaloneTopBarConfig;
  }
}
...
function appSwitcher(href: string, label: string): HTMLAnchorElement {
  ...
  link.className = 'aerie-app-switcher';
  link.href = href;
...
function themeSwitch(): HTMLFieldSetElement {
  const name = `aerie-theme-${(groupSeq += 1)}`;
  const current = readChoice();

  const fieldset = document.createElement('fieldset');
  fieldset.className = 'aerie-theme-switch aerie-theme-switch--accent';

  const legend = document.createElement('legend');
  legend.className = 'aerie-theme-switch__legend';
  legend.textContent = 'Theme';
  fieldset.appendChild(legend);

  for (const option of THEME_CHOICES) {
    const label = document.createElement('label');
    label.className = 'aerie-theme-switch__option';
...
export function createTopBar(config: StandaloneTopBarConfig): HTMLElement {
  ...
  header.className = 'aerie-topbar';

  const inner = document.createElement('div');
  inner.className = 'aerie-topbar__inner';

  const start = document.createElement('div');
  start.className = 'aerie-topbar__side';
  ...
  name.className = 'aerie-topbar__name';
  name.textContent = appName;
  start.appendChild(name);

  const end = document.createElement('div');
  end.className = 'aerie-topbar__side aerie-topbar__side--end';
  if (theme === 'switch') {
...
/**
 * Auto-mount from `window.aerieTopBar`.
 *
...
function autoMount(): void {
  const config = window.aerieTopBar;
  if (config) mountTopBar(config);
}

if (typeof window !== 'undefined') {
  if (window.aerieTopBar) applyHostTheme(window.aerieTopBar);
