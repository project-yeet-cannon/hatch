...
public class NwsAlertProviderTests
{
    ...
    [Fact]
    public async Task MapsAnAlertIntoAerieVocabulary()
    {
    ...
    {
        // NWS extends the CAP vocabulary; an alert whose severity Aerie cannot
    ...
    public async Task IdentifiesItselfWithTheConfiguredContact()
    {
        ...
        var (provider, handler) = NewProvider(Ok(), contact: "aerie@example.com");

        await provider.GetActiveAlertsAsync(40.0, -105.0, CancellationToken.None);

        Assert.Equal("Aerie/1.0 (aerie@example.com)", Assert.Single(handler.Requests).Headers["User-Agent"]);
    }
    ...
    public async Task StillIdentifiesItselfWhenNoContactIsConfigured()
    {
        ...
        Assert.Equal("Aerie/1.0 (self-hosted)", Assert.Single(handler.Requests).Headers["User-Agent"]);
    }
