...
export function PageHeader({ title, description, actions, level = 1, className }: PageHeaderProps) {
  ...
  const classes = ['hatch-page-header'];
  if (description) classes.push('hatch-page-header--described');
  if (className) classes.push(className);

  return (
    <header className={classes.join(' ')}>
      <div className="hatch-page-header__titles">
        <Heading>{title}</Heading>
        {description ? <p className="hatch-page-header__description">{description}</p> : null}
      </div>
      {actions ? <div className="hatch-page-header__actions">{actions}</div> : null}
    </header>
  );
}
