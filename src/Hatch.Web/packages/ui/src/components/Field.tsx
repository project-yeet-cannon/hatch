...
export function Field({ label, hint, error, as = 'label', className, children }: FieldProps) {
  const Control = as;
  const classes = ['hatch-field'];
  if (className) classes.push(className);

  return (
    <div className={classes.join(' ')}>
      <Control className="hatch-field__control">
        <span className="hatch-field__label">{label}</span>
        {children}
      </Control>
      {error ? (
        <span className="hatch-field__error" role="alert">
          {error}
        </span>
      ) : hint ? (
        <span className="hatch-field__hint">{hint}</span>
      ) : null}
    </div>
  );
}
