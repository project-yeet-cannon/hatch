...
public class CalendarSyncServiceTests
{
    ...
    public async Task AThrowingAccountLeavesTheOtherAccountsEventsIntact()
    {
        ...
        // Failure or not, "last synced" answers "when did Hatch last try".
    ...
    public async Task DisabledAndUnauthorizedAccountsAreNotFetched()
    {
        ...
        var service = NewService(new AppDbContext(options), client, new FakeTimeProvider(Now));
    ...
    private static CalendarSyncService NewService(
        AppDbContext db, IGoogleCalendarClient client, FakeTimeProvider time, string timeZone = NewYork, int agendaDays = 2) =>
        new(db, client,
    ...
    private static (CalendarSyncService Service, AppDbContext Db, Harness Harness) NewService(
    ...
    private static async Task Insert(AppDbContext db, string providerCalendarId, EfCalendarEvent e)
    {
    ...
    private static async Task<List<EfCalendarEvent>> Events(AppDbContext db) =>
        await db.CalendarEvents.AsNoTracking().OrderBy(e => e.ProviderEventId).ToListAsync();

    private static async Task<EfCalendarAccount> Account(AppDbContext db, Guid? id = null) =>
        id is { } known
