...
using Hatch.Api.Tests.ClimateControl;
...
public class PanelCommandTests
{
    ...
    private static async Task<(EfPanel Panel, EfPanelItem Item)> ThermostatAsync(
        AppDbContext db, bool withPower, decimal? minF = null, decimal? maxF = null, decimal? stepF = null)
    {
    ...
    private static async Task<(EfPanel Panel, EfPanelItem Item)> AddPanelAsync(AppDbContext db, EfPanelItem item, string name = "Climate")
    {
    ...
    private sealed record Harness(PanelsController Controller, AppDbContext Db, FakeHomeAssistantCommandService Ha);
