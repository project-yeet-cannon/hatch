...
/// <summary>
/// Receives client-side log events from Hatch's frontend apps (dashboard/kiosk, admin, ...)
/// and writes them through the normal ILogger pipeline, so they flow into the existing
/// docker.* -> fluent-bit -> OpenSearch hatch-logs index alongside every other Hatch.Api
...
[ApiController]
[Route("api/ui-logs")]
public class UiLogsController(ILogger<UiLogsController> logger, IHatchRevision revision) : ControllerBase
{
    [HttpPost]
    public IActionResult Post([FromBody] UiLogEntry[] entries)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            // reads to override the container-level default `service` (hatch-api)
            // with the specific frontend app for these lines.
            //
            // {HatchRevision} is the same trick for the same reason, and it is the
            // whole of "the emitter owns the field" (docs/plans/version.md): every
            // record's `hatch_revision` means "the revision of whatever `service`
            ...
            // record is hatch-api's - and the one query an administrator wants,
            ...
            logger.Log(
                level,
                "{Service}[{SessionId}] {Message} ({Url}) :: {Metadata} :: ip={ClientIp} device={DeviceId} rev={HatchRevision} seq={HatchSequence} relay={HatchRelayRevision} actor={ActorId} person={PersonId} personName={PersonName}",
                entry.App,
                entry.SessionId,
                entry.Message,
                entry.Url,
    ...
    private string? ClientRevision(UiLogEntry entry) =>
        string.IsNullOrWhiteSpace(entry.Revision)
            ? NullIfBlank(Request.Headers[HatchRevisionMiddleware.ClientHeaderName].ToString())
            : entry.Revision;

    private int? ClientSequence(UiLogEntry entry)
    {
        if (entry.Sequence is > 0) return entry.Sequence;
        return int.TryParse(Request.Headers[HatchRevisionMiddleware.ClientSequenceHeaderName].ToString(), out var parsed) && parsed > 0
            ? parsed
            : null;
    }
