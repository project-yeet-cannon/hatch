...
/// instance (charts/hatch's go2rtc Deployment) serves the same RTSP sub-stream
...
///
/// go2rtc is not the source of the camera's *address* either. Hatch holds that,
...
/// Hatch request, and never learns a camera's address or password - which are
...
) : ControllerBase
{
    ...
    public async Task Stream(Guid deviceId, CancellationToken ct)
    {
        ...
        // Where this camera's video actually comes from. Hatch holds it, not
