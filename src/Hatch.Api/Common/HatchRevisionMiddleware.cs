...
/// <summary>
/// Puts <c>Hatch-Revision</c> on every response this process sends, and notices
...
/// only <c>X-Hatch-*</c> header in the codebase is Traefik's forwardAuth grant,
...
/// during a rollout.
/// </summary>
public class HatchRevisionMiddleware(
    RequestDelegate next,
    IHatchRevision revision,
    ILogger<HatchRevisionMiddleware> logger,
    TimeProvider time)
{
    public const string HeaderName = "Hatch-Revision";

    /// <summary>What a web app sends back about its own build (see the Vite plugin).</summary>
    public const string ClientHeaderName = "Hatch-Client-Revision";
    public const string ClientSequenceHeaderName = "Hatch-Client-Sequence";

    /// <summary>
    /// The native kiosk shell's own build, which drifts independently of the
    /// bundle it is displaying and so travels separately.
    /// </summary>
    public const string ShellHeaderName = "Hatch-Shell-Revision";
