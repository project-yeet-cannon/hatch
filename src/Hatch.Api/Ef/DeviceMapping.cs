...
/// Hatch owns the camera's address and credential; go2rtc is told about a
...
public static class SiteSettingKeys
{
    ...
    /// <summary>OAuth client credentials for the operator's own Google Cloud project, used to connect family calendars - see CalendarOAuthController. Operator-supplied rather than shipped, since Hatch redeploys to other households (docs/ethos.md).</summary>
    ...
    /// (docs/plans/immich.md v+2) has Hatch proxying it rather than the kiosk
    ...
    /// never leaves the API: the kiosk asks Hatch for a photo and Hatch asks
