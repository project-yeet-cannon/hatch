...
/// into Hatch, so the admin import flow (Phase 3) can show what's left to map.
/// </summary>
public class DiscoveryService(TemplateClient template, AppDbContext db, IHomeAssistantStateReader stateReader) : IDiscoveryService
{
    ...
    // move, which is what lets Hatch stop being told a static address. It is
    ...
    /// <summary>A media_player.* entity: bare entity state ("playing"/"paused"/"idle"), no sub-attribute, read-write (DevicesController.PlayMedia). The speaker's switch.*/number.* tuning siblings (loudness, bass, balance...) are deliberately left unmapped - they're setup knobs, not things Hatch drives.</summary>
