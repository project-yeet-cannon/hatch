...
-- table from scratch if it is not already present.
--
-- The upstream reference script (linked above) guards this behind a DropDb
-- flag toggled by hand for an interactive DROP-then-recreate reset; there is
-- no such toggle here; and a hardcoded DO $$ ... $$ block is also what Flux's
-- postBuild.substituteFrom mangles - it collapses the literal `$$` down to a
-- single `$`, which is a syntax error to psql. Deleting the always-false
-- branch fixes that at the source instead of renaming the dollar-quote tag
-- to dodge it.
