...
- [ ] **Enable MAC address spoofing on each vNIC** — without it the CNI silently
      drops pod traffic. Classic Hyper-V failure, painful to diagnose after the fact
- [ ] Second fixed-size VHDX per VM (or physical disk passthrough) on the TB
      storage, for Longhorn
- [ ] VM auto-start on host boot; **Automatic Stop Action = Shut Down, not Save
      State** — saved-state VMs resume with clock skew that confuses etcd and certs
- [ ] NTP from pfSense on all three
